...
<template>
  <dialog ref="modalRef" class="fixed rounded-xl max-h-[95vh] w-[25vw] text-main-text border-none outline-none text-wrap bg-primary backdrop:bg-black/80 backdrop:backdrop-blur-sm">
    <img class="w-full" :src="props.personagem.image" :alt="props.personagem.name"/>
    <div id="informacoes-modal-personagem" class="px-5 py-3 border border-b-px border-tertiary">
    ...
    <div id="botao-fechar-modal" class="absolute cursor-pointer top-1 right-1 text-xl w-8 h-8 flex items-center justify-center text-main-text bg-black/50 rounded-full hover:bg-dead hover:text-white transition-all active:scale-90" aria-text="Fechar" @click="personagemStore.fecharModal">&#x2715;</div>
  </dialog>
</template>
